...
```
pcb_tiling_detection/
  README.md
  requirements.txt
  configs/
    default.yaml              ← all hyperparameters live here
  PCB_Defect/                 ← raw dataset (COCO JSON)
    images/
  ...
  data/
    processed/                ← auto-generated by Step 1
      ...
      dataset.yaml
      metadata.csv
    tiles/                    ← auto-generated by Step 1b
      images/{train,val}/
      labels/{train,val}/
      dataset.yaml
      tile_metadata.csv
  src/
    utils.py                  ← IoU, NMS, generate_tiles, gt_boundary_distance
    prepare_dataset.py        ← Step 1:  COCO → YOLO split + labels
    verify_dataset.py         ← Step 1b: dataset sanity checker + charts
    prepare_tiles.py          ← Step 1c: tile crops for tile-aware training
    train_detector.py         ← Step 2:  train YOLO12n
    infer_full.py             ← Step 3:  full-image inference
    infer_tiles.py            ← Step 4+5: tile inference + NMS
    topology_merge.py         ← Step 6:  TA-TM merging
    evaluate.py               ← Step 7:  mAP, recall, boundary/size tables
    analyze_resolution.py     ← Step 8:  resolution-collapse analysis + charts
    visualize.py              ← Step 9:  comparison figures
  outputs/
    runs/
      best_weights.txt        ← pointer updated after every training run
      train_tiles/            ← tile-trained YOLO12n artefacts
        weights/best.pt
        results.csv
        confusion_matrix.png
    predictions/              ← one CSV per method (9 files)
    tables/                   ← main_results.csv, size_recall.csv, boundary_recall.csv
    figures/
      summary_chart.png
      comparison_grid/        ← per-image TP/FP/FN overlays
      boundary_artifact/      ← zoomed tile-boundary comparisons
      resolution/             ← collapse histogram, recall-by-area, example crops
      verify/                 ← dataset verification samples
```
